...
export default function BASTPage() {
  ...
  const loadBasts = async () => {
    const data = await getBasts();
    setBasts(data);
  };
  ...
  return (
    <div className="flex flex-col gap-4 p-4 max-w-[1600px] mx-auto bg-white dark:bg-[#111111] min-h-screen transition-colors">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Berita Acara Serah Terima - Dokumen serah terima barang/jasa antara vendor dan klien.
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <BASTStatCard
          title="Companies"
          value={String(new Set(basts.map((b) => b.coverInfo.companyName)).size)}
